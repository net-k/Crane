...
public class BreadController : MonoBehaviour
{
    ...
    [FormerlySerializedAs("nextRepublicController")] [SerializeField] BreadController nextBreadController;
    ...
    {
        BreadController tmpBreadController2 = Instantiate(nextBreadController, InputPosition, Quaternion.identity);
        tmpBreadController2.SetRepublicInit();
    }
    ...
    void OnCollisionEnter2D(Collision2D collisionInfo)
    {
        ...
        BreadController tmpBreadController = collisionInfo.gameObject.GetComponent<BreadController>();

        if(republicNumber == tmpBreadController.GetRepublicNmber())
        {
            if(!(tmpBreadController.GetHitRepublicIDNumber() == -1 || tmpBreadController.GetHitRepublicIDNumber() == idNumber)) return; //当たった相手の、衝突した相手のIDNumberが-1(未接触)か自分自身でない場合はリターン

            hitRepublicIDNumber = tmpBreadController.GetIDNmber();

            //ヒットした相手のIDをリストに登録する
            mainManager.SetHitBreadIDNumber(idNumber, hitRepublicIDNumber);
        }
    }
}
